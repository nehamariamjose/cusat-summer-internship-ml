...
`temp\_vanilla.csv` — morning/afternoon/evening temperature readings, 

predicting `next\_day\_temp`.



\## Architecture

\- Input (T=3, features=1) → SimpleRNN(hidden\_size=8, tanh) → Dense(1, Linear)
...
> (`batch\_size=1`) SGD training over many epochs. Values above are from a 

> representative run; expect minor fluctuation (\~±0.5 MSE) if re-run.



!\[Actual vs Predicted — Scratch](assets/predictions\_scratch.png)

!\[Actual vs Predicted — Keras](assets/predictions\_keras.png)
