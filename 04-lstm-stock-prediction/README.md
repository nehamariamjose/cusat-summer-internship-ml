...
AAPL daily closing prices (`data/stock\_data.csv`), using a 5-day sliding window of prior closes (`WINDOW\_SIZE = 5`) to predict the next day's close.



\## Architecture

\- \*\*Input\*\*: 5-day sliding window of closing prices (`WINDOW\_SIZE = 5`), one feature per day.
...
!\[Scratch LSTM: actual vs predicted closing price](assets/lstm\_scratch\_results.png)

\*Actual vs. predicted closing price on the test set, scratch NumPy LSTM.\*



!\[Keras LSTM: actual vs predicted closing price](assets/lstm\_keras\_results.png)
